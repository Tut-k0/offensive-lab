...
Deployment should be complete within a few minutes. If you did not specify a DSRM password, the autogenerated one will be displayed in the output for optional saving.
The script will restart the computer 15 seconds after deployment completes.

### AD Environment Population
After reboot, log in as `DOMAIN\Administrator` with the password you set during deployment. We can then run the population script:
```powershell
.\Populate-ADEnvironment.ps1 -ConfigFile .\ad-populate-config.json

# Or with defaults
.\Populate-ADEnvironment.ps1
```

This script creates the following:
- Organizational Units (OUs)
- Security Groups (IT Admins, HR Staff, Finance Team, Sales Team, etc.)
- User Accounts (10 users across departments)
